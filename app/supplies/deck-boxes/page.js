...
function ProductGrid() {
  const { products, loading } = useProducts("Deck Boxes");
    ...
    <div className="bg-black min-h-screen py-12 px-4">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <Link
            href="/supplies/tcg-supplies"
            className="text-gray-500 hover:text-yellow-400 text-sm transition-colors"
          >
            TCG Supplies
          </Link>
          <span className="text-gray-500 text-sm mx-2">/</span>
          <span className="text-white text-sm">Deck Boxes</span>
        </div>
        <h1 className="text-white text-3xl font-bold mb-8">Deck Boxes</h1>
        <ProductGrid />
      </div>
    </div>
  );
